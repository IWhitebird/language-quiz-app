...
const Card = ({quiz} : {quiz : IQuiz}) => {
  return (
    <div className="border-[3px] border-dashed shadow-md shadow-black sha p-3 rounded-sm border-black w-full h-[400px] lg:w-[350px] lg:h-[380px] hover:scale-[1.1] transition-all ease-in-out duration-300 ">
        <img src={quiz.image} className="w-[100%] h-[40%] mx-auto" alt="img"/>

        <div className="w-[90%] mx-auto">
          <hr className="mt-1" />
          <h1 className="text-3xl font-bold mt-3">{quiz.name}</h1>
          <h2 className="text-xl">{quiz.description.slice(0 ,50)} {quiz.description.length >= 50 ? '...' : ''}</h2>
